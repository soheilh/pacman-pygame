    ...
    for y, row in enumerate(level):
        for x, tile in enumerate(row):
            if tile == '1' or tile == '2':
                ...
                top = (y > 0) and (level[y - 1][x] in wall_letter)
                bottom = (y < max_y) and (level[y + 1][x] in wall_letter)
                left = (x > 0) and (level[y][x - 1] in wall_letter)
                right = (x < max_x) and (level[y][x + 1] in wall_letter)
                top_right = (y > 0 and x < max_x) and (level[y - 1][x + 1] in wall_letter)
                top_left = (y > 0 and x > 0) and (level[y - 1][x - 1] in wall_letter)
                bottom_right = (y < max_y and x < max_x) and (level[y + 1][x + 1] in wall_letter)
